...
| 2026-06-14 | 0.14 | **Web UI sidebar 重构** — 左侧 SessionSidebar 常驻（280px，标题 + 新建按钮 + 按日期分组的会话列表 + 当前活跃高亮 + 设置入口）；右侧主内容区根据路由切换（WelcomePage / SessionPage / HistoryPage / SettingsPage）；点侧边栏会话即时切换，无需回主界面；SessionPage/HistoryPage/SettingsPage 去掉返回链接；6/6 E2E、20/20 vitest、web build 均通过。 |
