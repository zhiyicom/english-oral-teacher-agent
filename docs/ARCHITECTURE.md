...
| 2026-06-14 | 0.15 | **v0.9 语音 I/O** — STT 通过浏览器 `SpeechRecognition` API（Chrome 内置），🎤 按钮在输入区，持续收音 + interim 实时显示 + final 填入输入框；TTS 通过 `SpeechSynthesis` 朗读 LLM 回复，受 voice_enabled/speed/accent 控制；Settings 语音控件全面启用；零新依赖、零 API 成本。6/6 E2E、web build 通过。 |
