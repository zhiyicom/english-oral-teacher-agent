...
| 2026-06-14 | 0.12 | **v0.8.5 后打磨 (Phase 推进 + Prompt 增强)** — 30 分钟到不再硬终止，改为走 LLM 告别轮（与用户 "stop" 同路径）；SOUL.md 新增 4 阶段行为指引 + WARM_UP 跨会话多样化；新建 `prompts/topic-library.md`（A1/B1/B2 三级话题库），通过 loader 注入到 system prompt 的 `# TOPIC_LIBRARY` 部分；阶段行为指令从 SOUL.md 移入 `[System Context]` 动态块，每个阶段显示 `## You are in X phase` 指令头；阶段提醒注入用户消息前缀（`[Phase: END — say goodbye now]`）以增强 chat 模型遵从度；沉默跳过 WRAP_UP 时补一个 catch-up WRAP_UP 轮次（silence ≥10 min + 跳过 WRAP_UP → 先总结再告别）；`DEBUG_LOG_LLM=1` 环境变量记录完整 LLM 请求到 `data/llm-debug/`；修复 test 环境 `RUN_LIVE_LLM` 泄漏。53/53 vitest 通过。 |
