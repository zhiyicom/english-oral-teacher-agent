...
| 2026-06-15 | 0.16 | **v0.9.1 功能打磨** — Phase 首次切换注入完整 Context 前缀打断 LLM 惯性；END 阶段立即结束防无限告别循环；30 分钟优雅结束；catch-up WRAP_UP；话题库完全重建（30 话题对齐分类体系）+ countScore 评分 + Web UI 话题编辑器 + GET/PUT /api/topics；lastReview 动态查询；Web 会话结束 summarize + markEnded + embedding 流水线；profile-extractor 自动更新 USER.md；侧边栏删除会话；tool 标签过滤；AGENTS 合并入 SOUL；阶段指令移至 phases.md；DEBUG_LOG_LLM。22/22 vitest 通过。 |
