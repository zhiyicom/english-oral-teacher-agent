...
- **模板**：`prompts/USER.md.template`（commit 进 git）
...
> **阈值说明**：topic.keywords 数组通常 3-6 个词，session.keywords 3-8 个词。0.15 是宽松阈值——只要有 1 个词命中就匹配。如果发现误匹配太多，可调到 0.25。  
...
| 步骤 | 规则 | 行为 |
|---|---|---|
| 1. 硬排除 | `topic_stats.last_discussed_at > date('now', '-30 days')` | 直接踢出候选池 |
...
> 三个信号（时间、计数、兴趣）共同作用，但**时间是最硬的**——30 天内讲过的绝对不会重讲；计数和兴趣是软偏好。
...
```
[1] System: SOUL.md 内容
[2] System: AGENTS.md 内容
[3] System: USER.md 渲染
[4] System: [System Context] 块（动态；WARM_UP 前含历史摘要、待办等）
[5] Tool results（如有）
[6] User: 当前 session 完整对话
[7] User: 当前消息
```

> 与旧版的本质区别：`[6]` 是**当前 session 的完整对话**（从 session 开始到当前 turn），不是滚动窗口。
...
- [ ] 关键词自动与 topic library 匹配（Jaccard > 0.15），命中后 `topic_stats.discussion_count += 1`
